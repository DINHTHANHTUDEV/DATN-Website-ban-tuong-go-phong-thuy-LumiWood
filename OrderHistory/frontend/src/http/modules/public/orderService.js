const USER_ORDER_STATUSES = ["PENDING_PAYMENT", "PROCESSING", "SHIPPED", "DELIVERED", "CANCELLED_BY_USER", "COMPLETED", "AWAITING_CONFIRMATION"];
...
const generateMockUserAddress = (type = "shipping") => {
  return {
    ...
    street: `${Math.floor(Math.random() * 800) + 100} Mock ${type === "shipping" ? "Delivery" : "Billing"} St.`,
    city: "Mockington",
    state: "MC",
...
const generateMockOrderBase = (orderIdInput) => {
  ...
  const items = Array.from({ length: Math.floor(Math.random() * 4) + 1 }, () => generateRandomUserOrderItem(orderId));
  const subtotal = parseFloat(items.reduce((sum, item) => sum + item.itemTotalPrice, 0).toFixed(2));
  const discountApplied = subtotal > 50 ? parseFloat((subtotal * (Math.random() * 0.15 + 0.05)).toFixed(2)) : 0;
  const shippingFee = subtotal > 30 && Math.random() > 0.3 ? parseFloat((Math.random() * 10 + 2).toFixed(2)) : 0;
  ...
  if (status === "SHIPPED" || status === "DELIVERED" || status === "COMPLETED") {
    ...
    estimatedDeliveryDate = new Date(orderDate.getTime() + deliveryDays * 24 * 60 * 60 * 1000).toISOString();
...
const generateMockOrderSummaryDTO = (orderIdInput) => {
  ...
  return {
    ...
    items: base.items.map(i => ({ productName: i.productName, quantity: i.quantity, unitPrice: i.unitPrice })),
...
const generateMockUserOrderDetailDTO = (orderIdInput) => {
  ...
  if (base.status !== "AWAITING_CONFIRMATION") {
    orderHistory.push({ status: "PROCESSING", date: new Date(new Date(base.orderDate).getTime() + 1000*60*60).toISOString() });
    if (base.status === "SHIPPED" || base.status === "DELIVERED" || base.status === "COMPLETED"){
      orderHistory.push({ status: "SHIPPED", date: new Date(new Date(base.orderDate).getTime() + 1000*60*60*24).toISOString() });
      if (base.status === "DELIVERED" || base.status === "COMPLETED"){
        orderHistory.push({ status: "DELIVERED", date: base.estimatedDeliveryDate || new Date(new Date(base.orderDate).getTime() + 1000*60*60*24*3).toISOString() });
      }
    }
  }
  ...
  return {
    ...
    paymentMethodName: ["MockCard Ending 1234", "MockWallet Balance", "Cash On Delivery (Mock)"][Math.floor(Math.random()*3)],
    customerNotes: Math.random() > 0.6 ? "Please deliver in the afternoon if possible (mock note)." : null,
