<template>
  <div class="admin-shipping-method-list-view">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <router-link :to="{ name: 'adminShippingMethodNew' }" class="btn btn-primary" aria-label="Thêm phương thức vận chuyển mới">
        <i class="bi bi-plus-lg"></i> Thêm Phương thức
      </router-link>
    </div>

    <div v-if="loading" class="text-center my-5" aria-live="polite" aria-busy="true"> ... </div>

    <div v-else-if="error" class="alert alert-danger" role="alert">{{ error }}</div>

    <div v-else-if="methods.length > 0" class="table-responsive card shadow-sm">
      <table class="table table-hover table-striped mb-0 align-middle" role="table">
          ...
          <tr v-for="method in methods" :key="method.id">
            ...
            <td class="text-center">
              <span
                ...
                :class="method.isActive ? 'text-bg-success' : 'text-bg-secondary'"
                :aria-label="method.isActive ? 'Hoạt động' : 'Ngừng hoạt động'"
              >
            ...
            <td class="text-center">
              <router-link
                ...
                aria-label="Sửa phương thức vận chuyển"
              >
              ...
              <button
                ...
                :aria-label="method.isActive ? `Ngừng hoạt động phương thức ${method.name}` : `Kích hoạt lại phương thức ${method.name}`"
              >
                <span v-if="togglingStatusId === method.id" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
    ...
    <div v-else class="alert alert-info text-center" role="alert">Chưa có phương thức vận chuyển nào.</div>
  </div>
</template>
...
import { getAdminShippingMethods, updateAdminShippingMethod } from '@/http/modules/admin/adminShippingService.js';
...
const fetchMethods = async (page = 0) => {
  ...
  try {
    ...
    currentPage.value = response.data.number + 1; // Backend trả page 0-based
...
const confirmToggleStatus = async (method) => {
  ...
  try {
    ...
    await fetchMethods(currentPage.value - 1); // page backend 0-based
  } catch (err) {
...
watch(
  () => route.query.page,
  (newPageQuery) => {
    const pageNumber = Number.parseInt(newPageQuery, 10);
    fetchMethods(isNaN(pageNumber) || pageNumber < 1 ? 0 : pageNumber - 1);
  },
  { immediate: true }
);
...
<style scoped>
/* Nếu muốn custom thêm style thì thêm ở đây */
</style>
