...
const generateFakeClientArticleSummary = (idOverride) => {
  ...
  const excerptWords = ["Lorem", "ipsum", "dolor", "sit", "amet", "consectetur", "adipiscing", "elit", "sed", "do", "eiusmod", "tempor", "incididunt"];
  let excerpt = "";
...
const generateFakeClientArticleDetail = (slug, inputData = {}) => {
  ...
  for(let i=0; i < 3 + Math.floor(Math.random()*3); i++){
    fullContent += `<p>${contentParagraphs[Math.floor(Math.random()*contentParagraphs.length)]}</p>\n`;
  }
  ...
  return {
    ...
    featuredImageUrl: inputData.featuredImageUrl || `https://picsum.photos/seed/publicdetail${baseId}/800/500`,
    publishedAt: inputData.publishedAt || new Date(Date.now() - Math.random() * 20000000000).toISOString(),
    ...
    relatedArticles: Array.from({length: Math.floor(Math.random()*3)+2}, () => generateFakeClientArticleSummary()),
    ...inputData
  };
};
